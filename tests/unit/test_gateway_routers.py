...
class TestChatRouter:
    ...
    def test_chat_endpoint_success(self, monkeypatch):
        ...
        mock_service = SimpleNamespace(
            chat=lambda req, client_ip: {"ok": True, "data": "response", "meta": {"model": "gpt-4"}}
        )

        request = SimpleNamespace(client=SimpleNamespace(host="127.0.0.1"))
        req = ChatReq(messages=[{"role": "user", "content": "Hello"}])

        result = chat(req, request, service=mock_service)

        assert result["ok"] is True
        assert result["data"] == "response"

    def test_chat_endpoint_empty_messages(self, monkeypatch):
        ...
        with pytest.raises(HTTPException) as exc:
            chat(req, request, service=None)

        assert exc.value.status_code == 400
        assert "messages must be a non-empty array" in exc.value.detail

    def test_chat_endpoint_service_exception(self, monkeypatch):
        """Test chat service exception raises 502."""
        from services.gateway.models import ChatReq
        from services.gateway.routers.chat import chat

        mock_service = SimpleNamespace(chat=lambda req, client_ip: (_ for _ in ()).throw(Exception("upstream error")))

        request = SimpleNamespace(client=SimpleNamespace(host="127.0.0.1"))
        req = ChatReq(messages=[{"role": "user", "content": "Hi"}])

        with pytest.raises(HTTPException) as exc:
            chat(req, request, service=mock_service)
    ...
    def test_embed_endpoint_success(self):
        ...
        mock_service = SimpleNamespace(embed=lambda req: {"ok": True, "vectors": [[0.1, 0.2]], "dim": 2})

        req = EmbedReq(texts=["hello", "world"])
        result = embed(req, service=mock_service)

        assert result["ok"] is True
        assert len(result["vectors"]) == 1
        assert result["dim"] == 2

    def test_embed_endpoint_service_exception(self):
        """Test embed service exception raises 502."""
        from services.gateway.models import EmbedReq
        from services.gateway.routers.chat import embed

        mock_service = SimpleNamespace(embed=lambda req: (_ for _ in ()).throw(Exception("embed error")))

        req = EmbedReq(texts=["test"])

        with pytest.raises(HTTPException) as exc:
            embed(req, service=mock_service)

        assert exc.value.status_code == 502
        assert "embed_error" in exc.value.detail

    def test_rerank_endpoint_success(self, monkeypatch):
        """Test successful rerank request."""
        from services.gateway.models import RerankReq
        from services.gateway.routers.chat import rerank

        def mock_call_reranker(query, documents, top_n):
            return {"results": [{"index": 0, "score": 0.9, "text": "doc1"}]}

        monkeypatch.setattr("services.gateway.routers.chat.call_reranker", mock_call_reranker)
    ...
    def test_rerank_endpoint_exception(self, monkeypatch):
...
class TestGraphRouter:
    ...
    def test_graph_probe_success(self):
        ...
        mock_service = SimpleNamespace(
            probe=lambda req, client_ip: {"ok": True, "mode": "json", "data": {"test": "data"}, "provider": "gpt-4"}
        )

        request = SimpleNamespace(client=SimpleNamespace(host="127.0.0.1"))
        req = GraphProbeReq(model="graph-extractor")

        result = graph_probe(req, request, service=mock_service)

        assert result["ok"] is True
        assert result["mode"] == "json"

    def test_graph_probe_exception(self):
        """Test graph probe exception raises 502."""
        from services.gateway.models import GraphProbeReq
        from services.gateway.routers.graph import graph_probe

        mock_service = SimpleNamespace(probe=lambda req, client_ip: (_ for _ in ()).throw(Exception("probe failed")))

        request = SimpleNamespace(client=SimpleNamespace(host="127.0.0.1"))
        req = GraphProbeReq(model="test-model")

        with pytest.raises(HTTPException) as exc:
            graph_probe(req, request, service=mock_service)

        assert exc.value.status_code == 502
        assert exc.value.detail["error"] == "upstream_probe_error"

    def test_graph_extract_success(self):
        ...
        mock_service = SimpleNamespace(
            extract=lambda req, client_ip: {"ok": True, "data": mock_data, "provider": "gpt-4", "schema_hash": "abc"}
        )

        request = SimpleNamespace(client=SimpleNamespace(host="127.0.0.1"))
        req = GraphReq(context="Some text")

        result = graph_extract(req, request, service=mock_service)

        assert result["ok"] is True
        assert result["provider"] == "gpt-4"

    def test_graph_extract_value_error(self):
        """Test graph extract with ValueError raises 400."""
        from services.gateway.models import GraphReq
        from services.gateway.routers.graph import graph_extract

        mock_service = SimpleNamespace(
            extract=lambda req, client_ip: (_ for _ in ()).throw(ValueError("invalid input"))
        )
        ...
        with pytest.raises(HTTPException) as exc:
            graph_extract(req, request, service=mock_service)

        assert exc.value.status_code == 400

    def test_graph_upsert_success(self):
        ...
        mock_service = SimpleNamespace(upsert=lambda req: {"ok": True, "nodes": 5, "edges": 3})

        req = GraphUpsertReq(data=GraphData(nodes=[], edges=[]))
        result = graph_upsert(req, service=mock_service)

        assert result["ok"] is True
        assert result["nodes"] == 5
        assert result["edges"] == 3

    def test_graph_upsert_runtime_error(self):
        """Test graph upsert with RuntimeError raises 503."""
        from services.gateway.models import GraphData, GraphUpsertReq
        from services.gateway.routers.graph import graph_upsert

        mock_service = SimpleNamespace(upsert=lambda req: (_ for _ in ()).throw(RuntimeError("neo4j unavailable")))

        req = GraphUpsertReq(data=GraphData(nodes=[], edges=[]))

        with pytest.raises(HTTPException) as exc:
            graph_upsert(req, service=mock_service)
    ...
    def test_graph_query_success(self):
        ...
        mock_service = SimpleNamespace(query=lambda req: {"ok": True, "records": [{"n": {"id": "1"}}]})

        req = GraphQueryReq(query="MATCH (n) RETURN n")
        result = graph_query(req, service=mock_service)

        assert result["ok"] is True
        assert len(result["records"]) == 1

    def test_graph_query_value_error(self):
        """Test graph query with ValueError raises 400."""
        from services.gateway.models import GraphQueryReq
        from services.gateway.routers.graph import graph_query

        mock_service = SimpleNamespace(query=lambda req: (_ for _ in ()).throw(ValueError("invalid query")))

        req = GraphQueryReq(query="INVALID")

        with pytest.raises(HTTPException) as exc:
            graph_query(req, service=mock_service)
...
class TestVectorRouter:
    ...
    def test_index_chunks_success(self):
        ...
        mock_service = SimpleNamespace(
            index_chunks=lambda req: {"ok": True, "upserted": 2, "dim": 768, "collection": "chunks"}
        )

        req = IndexChunksReq(chunks=[ChunkItem(doc_id="doc1", text="text1"), ChunkItem(doc_id="doc2", text="text2")])

        result = index_chunks(req, service=mock_service)

        assert result["ok"] is True
        assert result["upserted"] == 2
        assert result["dim"] == 768

    def test_index_chunks_value_error(self):
        """Test index chunks with ValueError raises 400."""
        from services.gateway.models import IndexChunksReq
        from services.gateway.routers.vector import index_chunks

        mock_service = SimpleNamespace(index_chunks=lambda req: (_ for _ in ()).throw(ValueError("chunks required")))

        req = IndexChunksReq(chunks=[])

        with pytest.raises(HTTPException) as exc:
            index_chunks(req, service=mock_service)

        assert exc.value.status_code == 400

    def test_index_chunks_runtime_error(self):
        """Test index chunks with RuntimeError raises 503."""
        from services.gateway.models import ChunkItem, IndexChunksReq
        from services.gateway.routers.vector import index_chunks

        mock_service = SimpleNamespace(
            index_chunks=lambda req: (_ for _ in ()).throw(RuntimeError("qdrant unavailable"))
        )
    ...
    def test_search_success(self):
        ...
        mock_service = SimpleNamespace(
            search=lambda req: {"ok": True, "hits": [{"id": "1", "score": 0.9, "payload": {}}]}
        )

        req = SearchReq(query="test query")
        result = search(req, service=mock_service)
    ...
    def test_search_runtime_error(self):
        ...
        mock_service = SimpleNamespace(search=lambda req: (_ for _ in ()).throw(RuntimeError("qdrant down")))

        req = SearchReq(query="test")

        with pytest.raises(HTTPException) as exc:
            search(req, service=mock_service)
    ...
    def test_retrieve_success(self):
        ...
        mock_service = SimpleNamespace(
            retrieve=lambda req: {"ok": True, "hits": [], "subgraph": None, "query_time_ms": 100}
        )

        req = RetrieveReq(query="test")
        result = retrieve(req, service=mock_service)
    ...
    def test_retrieve_exception(self):
        ...
        mock_service = SimpleNamespace(retrieve=lambda req: (_ for _ in ()).throw(Exception("retrieval failed")))

        req = RetrieveReq(query="test")

        with pytest.raises(HTTPException) as exc:
            retrieve(req, service=mock_service)
